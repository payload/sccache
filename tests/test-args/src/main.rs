...
#[derive(Debug)]
struct CliCalls {
    compiler_calls: Vec<Vec<OsString>>,
    after_calls: Vec<Vec<OsString>>,
}
...
impl ReadArgs {
    ...
    fn create_pdb_arg(&self, output_path: &PathBuf) -> Option<OsString> {
        if self.args.contains_key(&DebugInfo) && (self.multiple_inputs() || self.pdb_arg == None) {
    ...
    fn create_cli_calls(&self) -> CliCalls {
    ...
    fn handle_shared_pdb(&self) -> Vec<OsString> {
        match (self.multiple_inputs(), &self.pdb_arg) {
            (true, &Some(ref pdb_path)) => vec![
                OsString::from("touch"),
                OsString::from(pdb_path.as_os_str()),
            ],
            (_, &None) => vec![OsString::from("touch"), OsString::from("v140.pdb")],
            _ => vec![],
        }
    }

    fn read_rsp_path(&mut self, rsp_path: &str) {
...
fn main() {
    ...
    read_args.read_rsp_path("bar.rsp");
...
fn argument_to_os_string(argument: &Argument) -> Vec<OsString> {
    argument.clone().into_iter().collect()
}
